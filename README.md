...
- Agent-card tool/model policy checks around worker execution, emitting `agent_tool_use_approved`, `agent_tool_use_denied`, `agent_model_use_approved`, and `agent_model_use_denied` timeline events before external tool or Gemma model use.
...
- Provider adapters for Hugging Face Gemma 4, the default LiveKit/Gemma/Kokoro voice runtime, optional proprietary realtime adapters, Tavily, SerpAPI, provider-neutral reranking with a deterministic local default, an optional Rust retrieval-ranker subprocess provider, and the Codex `imagegen` boundary.
- Non-secret provider readiness report at `GET /api/provider-readiness`, surfaced in the cockpit for Gemma 4 HF endpoints, realtime audio, web search, and imagegen boundary checks.
- Durable provider smoke ledger at `POST /api/runs/{run_id}/provider-smoke`, surfaced in the cockpit with an explicit live-call toggle. By default it records local/deterministic proof, blocked configuration, source ids, realtime session ids, smoke-proof status, and provider readiness without calling external providers. Passing `execute_live_calls=true` runs configured Hugging Face Gemma, selected realtime, selected web-search, and local reranker smoke steps.
...
- Model routing ledger at `POST /api/runs/{run_id}/model-routing-ledger`, proving per run that Gemma 4/HF is the expert layer while realtime audio, web search, and imagegen remain separate provider/tool boundaries; deterministic fallback writing is flagged as non-publishable routing evidence.
- Provider operations ledger at `POST /api/runs/{run_id}/provider-ops-ledger`, summarizing model/tool policy decisions, Gemma usage, realtime sessions, provider fallbacks, artifact model provenance, latency classes, provider/end-to-end latency, fallback reasons, and provider-backed versus rehearsal/local-demo smoke status as a durable observability artifact.
...
- Foundation reference ledger at `GET /api/foundation/references`, mapping official Google, LangGraph/LangChain, pgvector, Hugging Face, OpenAI, ElevenLabs, Cartesia, and Anthropic/Claude guidance to architecture decisions and agent implications.
...
- Publishing-readiness gate at `POST /api/runs/{run_id}/publish-readiness`, combining artifacts, provenance completeness, Gemma 4/HF writing provenance, source quality/freshness, accepted retrieval evidence, claim status, audits, and open feedback into a ready/needs-review/blocked decision.
...
This is local edge proof only; real provider-backed voice readiness still requires LiveKit + Gemma/Kokoro smoke with user audio and generated assistant speech.
...
This reports LiveKit, Gemma/HF, Kokoro, Rust voice-edge, and context-pruning readiness. With `preflight_edge=true`, the backend proves the selected Rust edge transport before reporting readiness.
...
This records a `realtime_voice_timing_ledger` artifact for the correlated LiveKit/Gemma/Kokoro chain: agent ready, user speech start, user-turn commit, Gemma turn start, Gemma generation start, first text/audio output, and barge-in cancellation plus turn-cancelled proof. The ledger refuses to mark readiness from unrelated turns or mismatched response ids.

The Next.js product voice panel renders this ledger as stage-level proof rows plus latest-turn latency segments, so creators/operators can see which part of the LiveKit -> Gemma -> Kokoro -> barge-in chain is measured or still missing without reading raw JSON.
...
Copy `.env.example` to `.env`, then configure the providers you want. For local secrets, prefer ignored secret files such as `HF_TOKEN_FILE=.secrets/hf_token` and `TAVILY_API_KEY_FILE=.secrets/tavily_api_key`; direct `HF_TOKEN=` and `TAVILY_API_KEY=` also work when you are comfortable storing them directly in `.env`.

- `HF_TOKEN` plus either dedicated Gemma 4 endpoint URLs or the default Hugging Face router chat-completions route for text/chat expert agents. Native realtime Gemma/Kokoro audio smoke requires a valid dedicated `GEMMA4_MULTIMODAL_ENDPOINT_URL`; `GEMMA4_PRIMARY_ENDPOINT_URL` and the router chat route are text/chat expert routes only.
- `GEMMA4_REALTIME_LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`, `GEMMA4_MULTIMODAL_ENDPOINT_URL`, `GEMMA4_REALTIME_STREAM_GEMMA=true`, and either hosted `KOKORO_TTS_ENDPOINT_URL` or local Kokoro dependencies for the default non-OpenAI realtime voice stack.
- `RUST_VOICE_EDGE_BINARY_PATH` after building `services/voice-edge` so the LiveKit participant can use VAD-driven turn endpointing, detect barge-in, and cancel active Gemma/Kokoro work.
