...
- Fresh current branch check on 2026-05-28: `fix_20260528-live-postgres-gate` is pushed. Latest checked branch-push product CI completed green for `Branch name policy`, `Python backend`, `Next.js frontend`, and both Rust service jobs; `Live Postgres (PR/main/manual)` was skipped on the branch push and is now configured to run on pull requests before merge. Auto PR failed at draft PR mutation with GitHub `403`, REST PR lookup returned no open PR, `provider-proof-pr-create` returned `manual_required` / `github_token_unavailable`, and connector PR creation returned `403 Resource not accessible by integration`. Regenerate exact `--head-sha` and `--ci-url` immediately before opening or updating the PR. Current manual PR path: `https://github.com/DeconvFFT/Content-creator-optimizer/compare/main...fix_20260528-live-postgres-gate?expand=1`.
- Latest checked branch-head evidence before this note-sync commit: `fix_20260528-live-postgres-gate` was pushed at `353544a540ca15aaf2490e7dec55efe5e656445a`. CI run `https://github.com/DeconvFFT/Content-creator-optimizer/actions/runs/26645073144` completed `cancelled` because `Rust service (services/voice-edge)` received a hosted-runner shutdown while installing the Rust toolchain, before cargo cache, format, or tests ran. `Branch name policy`, `Python backend`, `Next.js frontend`, and `Rust service (services/retrieval-ranker)` completed `success`; `Live Postgres (PR/main/manual)` was skipped until PR/main/manual. Local focused verification for `services/voice-edge` passed (`cargo fmt --all -- --check`; `cargo test --locked`, 25 tests). REST PR lookup returned no open PR, Auto PR check `Draft PR after green branch CI` failed with the expected repository PR-mutation `403` annotation, local `GITHUB_TOKEN`/`GH_TOKEN` are unset, `gh` is unavailable, the GitHub connector cannot rerun the cancelled job (`403 Resource not accessible by integration`), and `provider-proof-pr-create` returns `manual_required` / `github_token_unavailable`. Treat the branch as not merge-green until the cancelled GitHub job is rerun successfully or a PR/main/manual run supersedes it. Regenerate exact `--head-sha` and `--ci-url` after any follow-up commit and immediately before PR creation/update. Current manual PR path: `https://github.com/DeconvFFT/Content-creator-optimizer/compare/main...fix_20260528-live-postgres-gate?expand=1`.
