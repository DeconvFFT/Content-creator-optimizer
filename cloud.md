...
- PR-time evidence must be regenerated, not read from a committed SHA line: run `provider-proof-pr-handoff` with the current `--head-sha` and latest product-CI `--ci-url` immediately before opening or updating a PR. The latest live check before this note-sync update found prior head `68d6eb152860393c53dbc7cb78b071031102b75b` product CI green at `https://github.com/DeconvFFT/Content-creator-optimizer/actions/runs/26645347544` for branch policy, Python backend, Next.js frontend, and both Rust service jobs; `Live Postgres (PR/main/manual)` was skipped on branch push and must pass on PR/main/manual before merge approval. Auto PR still failed with the expected repository PR-mutation `403` annotation, REST PR lookup returned no open PR, local `GITHUB_TOKEN`/`GH_TOKEN` are unset, `gh` is unavailable, the GitHub connector cannot rerun Actions jobs or create PRs (`403 Resource not accessible by integration`), and `provider-proof-pr-create` returns `manual_required` / `github_token_unavailable`. Current manual PR path: `https://github.com/DeconvFFT/Content-creator-optimizer/compare/main...fix_20260528-live-postgres-gate?expand=1`.
