...
3. Boundary gate: realtime audio handles dialogue; Gemma 4 experts handle reasoning and critique; imagegen handles raster assets.
...
9. Model routing gate: build a `model_routing_ledger` when manager review needs proof that Gemma/HF remains the expert layer while realtime audio, web search, and imagegen stay separate provider/tool boundaries.
10. Provider operations gate: build a `provider_operations_ledger` when manager review needs model/tool approvals, denials, Gemma usage, realtime sessions, provider fallbacks, and artifact model provenance in one place.
...
- Model routing status when Gemma/HF, realtime audio, web search, or imagegen boundaries affect the run
