...
| Test coverage | Foundation + provider-proof + browser | Fresh focused checks: repo workflow CI `6 passed`, stable Python CI script `232 passed, 30 skipped`; latest branch-head remote CI run `26362067044` passed on commit `ab39562` | **Strong locally + remote green** | Keep coverage current with external publication proof work |
| CI/CD | Scaffolded and pushed on feature branch | `.github/workflows/ci.yml` and `scripts/ci-python-stable-tests.sh` exist; Python sync uses `uv sync --locked`; CI installs Playwright Chromium; `tests/test_repo_workflow_ci.py` guards lockfile, PR proof-gate handoff, and LiveKit SDK dev-extra use; `uv.lock` tracked; local command logs ignored | **86** | Latest branch-head remote CI run `26362067044` passed; REST PR lookup returned no open PR and connector PR creation was previously blocked by GitHub `403`; branch-protection/auto-merge still needs GitHub-side setup |
| Python lint (ruff) | Baseline verified locally and remotely | `[tool.ruff]` in `pyproject.toml`; fresh `uv run ruff check tests/test_repo_workflow_ci.py` returned `All checks passed!`; latest branch-head remote CI run `26362067044` Python Ruff step passed | **85** | Keep as required CI gate |
...
| Git / release discipline | Feature branch pushed | `main` is seeded at `f45c0a3`; `feature/livekit-voice-proof-capture` is pushed at `ab39562`; remote CI run `26362067044` passed | **58** | Manual PR/merge, branch protection, auto-merge setup, and tagged release still pending because connector PR creation is permission-blocked |
