...
| Test coverage | Foundation + provider-proof + browser | Fresh focused checks: repo workflow CI `10 passed`, stable Python CI script `236 passed, 30 skipped`; branch-head remote CI was green for current pushed head at last live check | **Strong locally + remote green** | Keep coverage current with external publication proof work |
| CI/CD | Scaffolded and pushed on feature branch | `.github/workflows/ci.yml` and `scripts/ci-python-stable-tests.sh` exist; Python sync uses `uv sync --locked`; CI installs Playwright Chromium; `tests/test_repo_workflow_ci.py` guards lockfile, PR proof-gate handoff, manual `provider-proof-pr-handoff` docs, and LiveKit SDK dev-extra use; `uv.lock` tracked; local command logs ignored | **88** | Branch-head remote CI was green for current pushed head at last live check; REST PR lookup returned no open PR and connector PR creation still returns GitHub `403`; branch-protection/auto-merge still needs GitHub-side setup |
