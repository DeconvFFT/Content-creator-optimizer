...
| Test coverage | Foundation + provider-proof + browser | Fresh focused checks: foundation `23 passed`, system-design viewer browser `2 passed`, repo workflow CI + LiveKit timing `10 passed`, stable Python CI script `229 passed, 30 skipped`; prior full Python suite passed with skips | **Strong locally** | Remote CI re-run after dependency fix still pending |
| CI/CD | Scaffolded and pushed on feature branch | `.github/workflows/ci.yml` and `scripts/ci-python-stable-tests.sh` exist; Python sync uses `uv sync --locked`; CI installs Playwright Chromium; `tests/test_repo_workflow_ci.py` guards lockfile and LiveKit SDK dev-extra use; `uv.lock` tracked; local command logs ignored | **78** | Latest remote run `26360567043` failed only on missing `livekit` SDK before this fix; PR creation is blocked by GitHub connector `403`; needs new remote CI and branch-protection proof |
