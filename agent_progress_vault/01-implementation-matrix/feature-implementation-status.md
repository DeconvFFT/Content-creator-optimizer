...
| Git / release discipline | Current fix branch pushed | `main` includes PR #16; `fix_20260528-live-postgres-gate` is pushed, branch-push product checks were green at last live check, and `provider-proof-pr-handoff` is available for manual PR creation | **62** | Manual PR/merge, PR live Postgres check, branch protection, auto-merge setup, and tagged release still pending because connector PR creation is permission-blocked |
