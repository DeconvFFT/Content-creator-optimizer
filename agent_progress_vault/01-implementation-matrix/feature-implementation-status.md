...
| Test coverage | Foundation + provider-proof + browser | Fresh focused checks: repo workflow CI `6 passed`, stable Python CI script `232 passed, 30 skipped`; branch-head remote CI was green at last live check | **Strong locally + remote green** | Keep coverage current with external publication proof work |
| CI/CD | Scaffolded and pushed on feature branch | `.github/workflows/ci.yml` and `scripts/ci-python-stable-tests.sh` exist; Python sync uses `uv sync --locked`; CI installs Playwright Chromium; `tests/test_repo_workflow_ci.py` guards lockfile, PR proof-gate handoff, and LiveKit SDK dev-extra use; `uv.lock` tracked; local command logs ignored | **86** | Branch-head remote CI was green at last live check; REST PR lookup returned no open PR and connector PR creation was previously blocked by GitHub `403`; branch-protection/auto-merge still needs GitHub-side setup |
...
| Git / release discipline | Feature branch pushed | `main` is seeded at `f45c0a3`; `feature/livekit-voice-proof-capture` is pushed and branch-head remote CI was green at last live check | **58** | Manual PR/merge, branch protection, auto-merge setup, and tagged release still pending because connector PR creation is permission-blocked |
