...
- Latest token-aware helper attempt with branch head `3dc61a30a67598e87e4a273cc695c241f8250eea` and CI run `26577750258` returned `manual_required` / `github_token_unavailable`; local environment has no `GITHUB_TOKEN` or `GH_TOKEN`, so the next non-secret path is manual PR creation or repository Actions permission upgrade.
