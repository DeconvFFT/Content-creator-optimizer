...
| **Refreshed 2026-05-28: current PR and merge-gate handoff** | [[../cloud]] / [[../social_media_optimiser/wiki/ops/active-codex-context]] / [[06-live-voice/openrouter-livekit-current-unblock-guide]] | Current pushed branch is `fix_20260528-live-postgres-gate`; latest checked branch-push product CI passed, Auto PR failed at draft PR mutation with GitHub `403`, no open PR exists for the branch, `provider-proof-pr-create` returned `manual_required` / `github_token_unavailable`, and `Live Postgres (PR/main/manual)` is now configured to run on pull requests before merge. Exact `--head-sha` and `--ci-url` must be regenerated at PR creation time. | Create the manual PR from the current compare URL or provide PR mutation credentials/permissions, then wait for PR CI including live Postgres |
