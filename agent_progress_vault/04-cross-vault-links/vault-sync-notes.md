...
- Fresh read-only check `2026-05-24T21:23:57Z`: pushed head `e5deff47098281a089f55f2528a878cb7fec73d7` is 46 commits ahead of `main` / 0 behind. CI run `26372896706` is completed `success`; Branch name, Python backend, Next.js frontend, and both Rust service jobs passed, while Live Postgres was skipped by design. Auto PR run `26372896700` waited for that CI success, generated the no-secret PR body, then hit the expected `403` create/update denial; REST PR lookup returned no PRs for `feature/livekit-voice-proof-capture`. Anonymous branch-protection lookup now returns `401`, so repository settings still require an authenticated owner-side check.
