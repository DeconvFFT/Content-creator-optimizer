...
- **Publication blocked** — manual-publication policy acknowledgement, durable destination, and rollback/postcondition artifacts
...
- PR/CI handoff: feature branch remote CI was green for the pushed head at the last completed live check. The Auto PR workflow reached its create/update step after generating the no-secret provider proof PR body, but GitHub denied draft PR create/update with repository-settings `403`, so no PR was created. The warning-only 403 behavior is superseded by commit `c995e386e7bde9a2580ea22c99d3903b3dbcf8c0`: the workflow now emits `Auto PR failed` and fails the Auto PR job when PR mutation is denied. GitHub connector PR creation still returns `403 Resource not accessible by integration`, REST lookup showed no open PR at the last successful check, and public branch metadata reported `main` as unprotected. CI now runs `uv lock --check` before Python dependency sync and installs from committed `uv.lock`; keep local command logs untracked. Use `uv run all-about-llms-admin provider-proof-pr-handoff --run-id 190ae2f9-a74b-4a23-b39c-aaf2d636bd8e --operator-input-path social_media_optimiser/output/provider-proof/190ae2f9-a74b-4a23-b39c-aaf2d636bd8e/operator-inputs.template.env --branch <current-branch-name> --ci-url <latest-branch-head-ci-url> --head-sha <current-branch-head-sha>` for a no-secret manual PR body generated with fresh branch evidence. The filled branch must be the branch being opened, the filled CI URL must be a same-repo GitHub Actions run URL, and the filled SHA must be a 40-character hex commit id.
- Fresh PR integration check `2026-05-24`: branch-head CI completed green, Auto PR completed after the no-secret handoff path but no PR was created, GitHub connector PR creation still returned `403 Resource not accessible by integration`, and local `provider-proof-pr-create` still returns `manual_required` without `GITHUB_TOKEN` or `GH_TOKEN`. Manual PR compare remains <https://github.com/DeconvFFT/Content-creator-optimizer/compare/main...feature/livekit-voice-proof-capture?expand=1>. Regenerate `provider-proof-pr-handoff` with fresh `--branch`, `--ci-url`, and `--head-sha` at PR creation time.
- Fresh continuation check `2026-05-25`: the local branch is ahead by one handoff-sync commit after `c995e386e7bde9a2580ea22c99d3903b3dbcf8c0`, but current Codex network polling could not reach GitHub to push or verify final CI/Auto PR conclusions. Fresh local proof checks using `UV_CACHE_DIR=/private/tmp/all-about-llms-uv-cache` still report `provider-backed-live-voice-proof` accepted, `external-publication-proof` as latest failed, and strict operator-input readiness blocked on `LINKEDIN_POLICY_ACKNOWLEDGEMENT_ARTIFACT_ID`, `PUBLICATION_DURABLE_PLATFORM_ID_OR_URL`, and `PUBLICATION_ROLLBACK_OR_POSTCONDITION_ARTIFACT_ID` only. Keep using manual PR creation or repository Actions/app permission changes, and regenerate exact SHA/CI evidence after any follow-up commit or before opening the PR.
...
| **Refreshed 2026-05-31: current PR and merge-gate handoff** | [[../cloud]] / [[../social_media_optimiser/wiki/ops/active-codex-context]] / [[06-live-voice/openrouter-livekit-current-unblock-guide]] | Current pushed branch is `fix_20260528-live-postgres-gate` at `96ac8901ceadff69f45e9c2721ed5edde4d9048d`; branch-push product CI passed at `https://github.com/DeconvFFT/Content-creator-optimizer/actions/runs/26694610526`; current `main` is `2ee0ec464d54f68bedfa5f39e05b3ea22b2ee8eb`; and a non-destructive merge-tree check found no mechanical merge conflict. Auto PR run `26694610524` generated the no-secret handoff and then failed at draft PR mutation with GitHub `403`; REST lookup still finds no open PR; `provider-proof-pr-create` still returns `manual_required` / `github_token_unavailable`; and `Live Postgres (PR/main/manual)` remains skipped until PR/main/manual. Exact `--head-sha` and `--ci-url` must be regenerated at PR creation time after any follow-up commit. | Create the manual PR from the current compare URL or provide PR mutation credentials/permissions, then wait for PR CI including live Postgres |
| **Refreshed 2026-06-04: manual-post publication boundary** | [[../social_media_optimiser/wiki/ops/active-codex-context]] / [[06-live-voice/openrouter-livekit-current-unblock-guide]] / [[../docs/external-publication-proof-runbook]] | Publication readiness now has an explicit `manual_publication_mode=true` path: agents build a review-ready package, the operator reviews and manually posts to LinkedIn, then supplies durable policy-review, post URL/platform id, and rollback/postcondition evidence. `run-autonomous-pass --acknowledge-publish-channel-policy --manual-publication-mode` carries this through the content pipeline. No LinkedIn publication token is needed for this proof path. | Rerun package/readiness capture after local Postgres/Docker is available; do not start or restart shared runtime without operator approval |
| **Runtime recheck 2026-06-05: live capture blocked** | [[../social_media_optimiser/wiki/ops/active-codex-context]] / [[../system_design_vault/04-agent-studio-implications/agent-studio-objective-completion-audit]] | Docker daemon, FastAPI `127.0.0.1:8000`, and local Postgres were unavailable. Manual-post code/tests and ignored proof packets are refreshed, but `distribution-package.json` was not produced and live publish-readiness was not captured. | Restore/start the approved local runtime first, then rerun autonomous pass/package/build/readiness capture; no shared processes were restarted by this agent |
| **Branch evidence 2026-06-05: manual-post fix pushed** | [[../social_media_optimiser/wiki/ops/active-codex-context]] / `/private/tmp/provider-proof-pr-handoff-97365ea.md` | Commit `97365eae371b0066ae65e5b90c38165930aa3089` is pushed and branch CI run `27031932219` is green for product checks. Auto PR run `27031932223` failed with GitHub draft PR create/update `403`, and no open PR exists. | Create PR manually from the compare URL or enable Actions PR write permissions; `Live Postgres (PR/main/manual)` still needs PR/main/manual execution before merge |
