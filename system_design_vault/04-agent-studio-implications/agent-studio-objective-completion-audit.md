...
- Manual LinkedIn publication is now an explicit content/readiness mode, not an agent-posting or token-credential path. With `manual_publication_mode=true`, publish-readiness can pass after supported channel metadata and policy acknowledgement without requiring `LINKEDIN_ACCESS_TOKEN`; `run-autonomous-pass --acknowledge-publish-channel-policy --manual-publication-mode` carries the same contract through the content pipeline. The external proof still requires operator-supplied durable policy review, manual post URL/platform id, and rollback/postcondition evidence.
- 2026-06-05 implementation recheck: focused regressions passed for manual publish-readiness, autonomous-pass pass-through, proof-plan validation, and static proof surfaces. Live Phase A capture is not proven because Docker, backend, and local Postgres were unavailable; the package builder failed with Postgres `Connection refused` outside the sandbox, so distribution-package and publish-readiness artifacts must be regenerated after runtime/database availability.
...
- Verification strategy should stay risk-scaled rather than smoke-test-heavy. Use the narrow test for the touched behavior first, then only relevant Python/frontend/Rust/browser checks for that surface; new smoke suites or scaffolding need a concrete regression or proof-gate risk. Keep document access inside the project unless the operator explicitly asks for outside material or the task cannot be safely completed without it. Source: user directives on 2026-05-26 mirrored from the project vault active context and `AGENTS.md` verification baseline.
...
- 2026-05-31 PR-time evidence rule: committed notes must not be treated as current branch-head evidence after follow-up commits; regenerate the current `--head-sha` and latest product-CI `--ci-url` immediately before PR creation/update. The latest live check found remote branch head `96ac8901ceadff69f45e9c2721ed5edde4d9048d`, remote `main` `2ee0ec464d54f68bedfa5f39e05b3ea22b2ee8eb`, and product CI green at `https://github.com/DeconvFFT/Content-creator-optimizer/actions/runs/26694610526` for branch policy, Python backend, Next.js frontend, and both Rust service jobs, while `Live Postgres (PR/main/manual)` remained intentionally skipped until PR/main/manual. A non-destructive merge-tree check against current `main` produced tree `b3a377a4b7b7473804e430a8d449a28ac542f3ec`, so no mechanical merge conflict was found. REST PR lookup found no open PR, Auto PR run `26694610524` waited for matching CI success, generated the no-secret handoff, then failed with GitHub draft PR mutation `403`, local `provider-proof-pr-create` returned `manual_required` / `github_token_unavailable`, and the GitHub connector PR create attempt returned `403 Resource not accessible by integration`.
- 2026-05-30 proof-gate recheck: `provider-proof-completion-status` still reports accepted `provider-backed-live-voice-proof` and latest failed `external-publication-proof`; strict operator-input readiness still blocks only on placeholder `LINKEDIN_POLICY_ACKNOWLEDGEMENT_ARTIFACT_ID`, `PUBLICATION_DURABLE_PLATFORM_ID_OR_URL`, and `PUBLICATION_ROLLBACK_OR_POSTCONDITION_ARTIFACT_ID`.
...
This failed record is preserved as historical audit evidence from the prior native-Gemma proof lane. It has been superseded for current work by the accepted OpenRouter DeepSeek V4 Flash + LiveKit + Kokoro live-voice proof. External LinkedIn publication proof remains blocked on the three operator-owned manual-publication evidence inputs: policy acknowledgement artifact, durable publication URL or platform ID, and rollback/postcondition artifact. No LinkedIn token or credential file is required for this proof path.
