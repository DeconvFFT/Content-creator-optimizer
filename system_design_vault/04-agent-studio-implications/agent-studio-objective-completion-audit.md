...
- proof implication: live voice remains accepted, but the objective remains open. External publication is still the latest failed proof and readiness blocks on LinkedIn token file, policy acknowledgement artifact, durable destination URL/platform id, and rollback/postcondition artifact.
...
- proof recheck implication: 2026-05-25 local proof commands using a workspace-safe `/private/tmp` uv cache still report accepted OpenRouter/LiveKit/Kokoro live voice, `external-publication-proof` as latest failed, and strict operator-input readiness blocked on LinkedIn token file, policy acknowledgement, durable destination, and rollback/postcondition evidence.
