...
- A route becomes preflight-ready when `provider-backed-live-voice-proof.preflight-validation.json` is `valid_preflight_artifacts` for `livekit-transport`, `livekit-agent-participant`, `voice-agent-backend-event-sink`, `openrouter-live-dialogue-reasoning`, `kokoro-tts`, and `rust-voice-edge`. UUID `190ae2f9-a74b-4a23-b39c-aaf2d636bd8e` has since recorded accepted provider-backed live-voice proof for that route.
...
- Product UI must distinguish provider-route readiness from objective completion. Current live voice has accepted OpenRouter/LiveKit/Kokoro proof, but completion remains blocked until external publication proof is accepted and closure review passes.
