...
- CI jobs: branch-name policy, Python backend, Next.js frontend, Rust services, and `Live Postgres (PR/main/manual)`, which must run on PRs before merge and also runs on `main`/manual dispatch.
...
- `.github/workflows/auto-pr.yml` is the current repo-owned draft PR fallback: on `feature/**` and `fix_*` pushes it waits for matching branch CI success, synthesizes a temporary no-secret PR input file, generates `provider-proof-pr-handoff`, and creates or updates a draft PR with the Actions `GITHUB_TOKEN`. The temporary input keeps accepted live-voice fields configured with dummy local files while publication fields remain blocked by placeholders, so ignored proof output stays out of clean-runner PR creation. If GitHub returns 403, commit `c995e386e7bde9a2580ea22c99d3903b3dbcf8c0` changes the workflow to report `Auto PR failed` and fail the Auto PR job; the old warning-only green behavior is superseded. The repo setting still needs Actions read/write workflow permissions plus `Allow GitHub Actions to create and approve pull requests`.
- `provider-proof-pr-create` can attempt GitHub REST PR creation from the current no-secret proof handoff when a local `GITHUB_TOKEN` or `GH_TOKEN` exists. It now updates an existing open branch/base PR before creating a new draft PR, mirroring Auto PR workflow behavior. If no token is available, it emits compact manual-required JSON with the compare URL and handoff command, without printing credential values or the full Markdown body.
- Fresh 2026-05-24 PR dry-run evidence: the feature branch generated a draft PR request shape for `feature/livekit-voice-proof-capture` into `main` with no secret values printed. The latest post-push Auto PR run confirmed CI success and no-secret PR body generation, but GitHub repository settings denied draft PR create/update with `403`, so no PR exists yet. The intended committed dependency artifact is `uv.lock`; local command logs remain untracked.
- If GitHub PR creation is blocked by integration permissions, run `uv run all-about-llms-admin provider-proof-pr-handoff --run-id 190ae2f9-a74b-4a23-b39c-aaf2d636bd8e --operator-input-path social_media_optimiser/output/provider-proof/190ae2f9-a74b-4a23-b39c-aaf2d636bd8e/operator-inputs.template.env --branch <current-branch-name> --ci-url <latest-branch-head-ci-url> --head-sha <current-branch-head-sha>` and paste the generated no-secret body into a manual PR after filling the placeholders from the current branch head. This preserves provider-proof gate state, current OpenRouter/LiveKit/Kokoro routing, CI evidence, and manual-publication operator blockers without exposing secret values.
